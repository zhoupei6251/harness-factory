<!--
platform/claude/CLAUDE.md — Claude 入口文件的母版
渲染器将此文件复制到 `.harness/rendered/CLAUDE.md`，项目根 `CLAUDE.md` 用一行 @import 引入。
Claude 支持 @ 语法递归 import。
总预算 ≤240 行（design §6）。
-->

# Harness — Claude Entry

`@.harness/rendered/CLAUDE.md`

<!--
上面这一行 @import 是项目根 CLAUDE.md 的实际内容；
本文件是编译器渲染的产物（.harness/rendered/CLAUDE.md）。
Claude Code 读到 `.claude/rules/HARNESS.md`（同样 @import）时跟着这里走。
-->
...
## Claude 专属：leader 模式（L 档可选）

L 档批次可考虑切到 leader 模式：`claude --agent leader`。详见 `agents/leader.md`。

> **限制**（已查证）：subagent 拿不到 `AskUserQuestion`——子 agent 不能跟用户对话。门禁确认必须发生在主会话。
...
>
> Claude 接 hooks：session-start / post-tool-use / stop。详见 `.claude/settings.json` hooks 段（merge 不覆盖业务设置）。