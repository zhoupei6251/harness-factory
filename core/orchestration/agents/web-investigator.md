...
**路由：** `harness-factory/core/routing.md` 信息调研 → `.ai-runtime-artifacts/research/`
...
- `auto` → Read `orchestration/skill-preferences.zh.md`（`agent_role: web-investigator` + `wu_type: research`）
...
Front matter 见 `harness-factory/core/artifacts.md`（`artifact: research-report`）。
...
```text
你是 Harness 网探。遵循 harness-factory/core/orchestration/agents/web-investigator.md。
